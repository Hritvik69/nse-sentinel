...
def compute_score_mode6(row: dict) -> tuple[float, dict]:
    ...
    # RSI 53-59 sweet spot — controlled swing zone
    if 53 <= ri <= 59:    pts["Swing RSI Sweet Spot"]   = 22   # strongest for swing
    elif 59 < ri <= 63:   pts["RSI Acceptable"]         = 12

    # Volume — moderate only (controlled vol for swing)
    if 1.3 <= vol_r <= 2.0:  pts["Controlled Vol"]      = 16
    ...
    if ri > 62:              pts["RSI Above Swing Zone"] = -22
    if ri < 51:              pts["RSI Too Low"]          = -10
    if vol_r > 2.5:          pts["Sharp Vol Spike"]      = -16  # unique to swing
...
def check_bull_trap_mode6(row: dict) -> str:
    ...
    e50   = safe(row.get("EMA 50",          0))
    hits  = sum([ri > 63, vol_r > 2.5, r5d > 7.0, e20 < e50])
...
def predict_ml_mode6(row: dict) -> float:
    ...
    try:
        ...
        # ema_slope not available from row — use de20 as proxy (smaller = rising slope)
        eslope = max(0.0, 0.5 - de20 * 0.05)
